...
void test_entire() {
  ...
  for (int row = 0; row < rowcount; row++) {
    ...
    string predicted_class = maxProbClass(probs);

    // Compare predicted class with actual class and update correct count
    if (predicted_class == testData[row][attrcount - 1]) {
      correct++;
    }
  }

  double acc = static_cast<double>(correct) / rowcount * 100; // Calculate accuracy as percentage
